...
class UI(clisys.CLISys):
    ...
    @clisys.command(name='14', description='Get min cost path between vertices.')
    def display_min_cost_path(self, args):
        start_vertex = int(input("Input start vertex: "))
        end_vertex = int(input("Input end vertex: "))

        path = self.__service.get_min_cost_path(start_vertex, end_vertex)
    ...
    @clisys.command(name='15', description='Check if DAG.')
    def check_dag(self, args):
        sorted = self.__service.predecessor_count_sorting()

        if not sorted:
            print("Graph is not DAG.")
        else:
            print(f"Graph is DAG. {sorted}")

    @clisys.command(name='16', description='Get schedule.')
    def get_schedule(self, args):
        (sorted, start_time_earliest, end_time_earliest) = self.__service.get_schedule_earliest()
        (sorted, start_time_latest, end_time_latest) = self.__service.get_schedule_latest()

        for activity in sorted[1:]:
            print(f"Activity {activity} : Earliest {start_time_earliest[activity]}-{end_time_earliest[activity]} : Latest {start_time_latest[activity]}-{end_time_latest[activity]} ")
