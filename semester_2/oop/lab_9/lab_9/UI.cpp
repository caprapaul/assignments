...
void UI::myList(vector<string> arguments)
{
    if (this->serviceMode == "B")
    {
        ...
        string filePath = this->lowQualificationService.GetSaveFilePath();
        ...
        {
            system(string(("\"C:\\Program Files (x86)\\BraveSoftware\\Brave-Browser\\Application\\brave.exe\" ") + filePath).c_str());
        }
...
void UI::myListLocation(std::vector<std::string> arguments)
{
...
bool UI::processInput(string input)
{
    ...
    {
        this->myListLocation(commandArguments);
    }
