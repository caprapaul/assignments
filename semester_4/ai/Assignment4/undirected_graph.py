...
class UndirectedGraph:
    ...
    def add_edge(self, edge):
        ...
        if self.get_vertex(edge.start) is None:
            raise ValueError("Invalid source vertex.")

        if self.get_vertex(edge.end) is None:
            raise ValueError("Invalid destination vertex.")

        self.__edges[(edge.start, edge.end)] = edge
        self.__vertex_edges[edge.start].append(edge)
        self.__vertex_edges[edge.end].append(edge)
