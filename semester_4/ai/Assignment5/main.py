...
class Mecanism():
    def __init__(self):
        ...
        # pendulum angle from vertical (down) in radians
        self.theta = 0.22 * pi
    ...
    def dynamics(self, F=0):
        ...
        self.d2theta += -(self.m * self.l * (self.dtheta ** 2)
                          * cos(self.theta) * sin(self.theta))/(self.M + self.m)
        self.d2theta += -cos(self.theta) * F / (self.M + self.m)
        self.d2theta = self.d2theta / \
            (self.l * (4/3 - (self.m * (cos(self.theta)**2))/(self.M+self.m)))
        ...
        self.t = self.t + self.dtheta * self.scaleTime + \
            0.5 * self.d2theta * (self.scaleTime ** 2)
        ...
        newX = self.x + self.dx * self.scaleTime + \
            0.5 * self.d2x * (self.scaleTime ** 2)
...
def test():
    ...
    while 1:
        ...
        #appliedForce += random.randrange(-100, 100)
